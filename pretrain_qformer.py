#!/usr/bin/env python3
"""Recipe for training a Transformer ASR system with QFormer compression.

Same pipeline as pretrain.py but uses QFormer (fixed-rate compression via
chunked mean-pool queries + cross-attention) instead of BoundaryPredictor.

To run this recipe:
> python pretrain_qformer.py hparams/qformer/2x_4x.yaml
"""
...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if flags.PRINT_FLOW:
            print(f"[pretrain_qformer.py] AFTER QFormer:")
        if flags.PRINT_DATA:
            print(f"  enc.shape = {enc.shape}")
            print(f"  enc_lens.shape = {enc_lens.shape}")
            print(f"  enc_lens = {enc_lens}")
        ...
        # === End QFormer Compression ===

        if flags.PRINT_FLOW:
            print(f"[pretrain_qformer.py] BEFORE Transformer:")
        if flags.PRINT_DATA:
            print(f"  enc.shape = {enc.shape}")
    ...
    def on_stage_start(self, stage, epoch):
        ...
        # No temperature scheduling needed for QFormer
