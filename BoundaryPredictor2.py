...
def common(boundaries):
    """
    Arithmetic segment-distance matrix that preserves gradient flow.

    Uses cumsum + subtraction (no boolean comparisons) so gradients
    from the STE on boundaries propagate through to the boundary MLP.

    Returns foo [B, L, S] where foo[b,l,s] == 0 iff position l belongs
    to segment s.  None when there are no boundaries.
    """
...
class BoundaryPredictor2(nn.Module):
    ...
    def _attention_pooling(self, boundaries, hidden, lengths):
        ...
        # Step 2: Build gradient-carrying soft mask from foo
        # foo == 0 at in-segment positions (from arithmetic cumsum).
        # (1 - foo) has value 1.0 there but gradient -1 w.r.t. foo,
        # which chains back through cumsum -> boundaries -> STE -> probs -> MLP.
        out_of_segment = (foo.detach() != 0)
        soft_mask = torch.where(
            out_of_segment, torch.zeros_like(foo), 1.0 - foo)  # [B, L, S]
        ...
        # Apply length mask to soft_mask
        soft_mask = soft_mask * length_mask.unsqueeze(-1)
        ...
        # Step 6: Apply gradient-carrying log-bias instead of hard -inf mask
        # log(soft_mask) at in-segment positions: log(1-foo) where foo=0
        #   -> value = 0 (no effect on attention), gradient = -1 (flows back!)
        # log(soft_mask) at out-of-segment positions: log(eps) ≈ -18
        #   -> effectively -inf, suppresses attention
        log_bias = torch.log(soft_mask.clamp(min=1e-8))
        log_bias_transposed = log_bias.permute(
            0, 2, 1).unsqueeze(1)  # (B, 1, S, L)
        attn_scores = attn_scores + log_bias_transposed
