...
between them. Decoding is performed with attention-based beamsearch coupled with a neural
language model.

To run this recipe, do the following:
> python pretrain_test.py hparams/no_ctc_4x.yaml
...
The neural network is trained with seq2seq loss only (no CTC) and sub-word units
estimated with Byte Pairwise Encoding (BPE) are used as basic recognition tokens.
...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if use_bp:
            ...
            # Compute target boundary counts based on target_phonemes flag
            if stage == sb.Stage.TRAIN:
                # Check if we should use phoneme-based or fixed-level compression
                ...
                if use_phoneme_targets:
                    ...
                    if flags.PRINT_DATA:
                        ...
                        print(
                            f"[Prior Targets] target_counts: {target_boundary_counts[:5]}")
            else:
        ...
        return p_seq, enc_lens, hyps

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (seq2seq only, no CTC) given predictions and targets."""

        (p_seq, enc_lens, hyps) = predictions
