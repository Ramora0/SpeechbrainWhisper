#!/usr/bin/env python3
"""Ablation round 2 training script.

Based on pretrain.py with added compression scheduling support.

Compression schedule types (set via compression_schedule_type hparam):
    none              - Fixed compression throughout (default, same as pretrain.py)
    linear            - Linear ramp from 1x to target over all epochs
    hold_then_linear  - Hold at 1x for N epochs, then linear ramp to target
    hold_at_target    - Ramp from 1x to target over N epochs, then hold
    step              - Discrete compression steps at specified epochs
"""
...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if flags.PRINT_FLOW:
            print(f"[pretrain_test.py] BEFORE BoundaryPredictor:")
        if flags.PRINT_DATA:
            print(f"  enc.shape = {enc.shape}")
            print(f"  enc_lens.shape = {enc_lens.shape}")
            print(f"  enc_lens = {enc_lens}")

        # Compute target boundary counts using scheduled prior
        if stage == sb.Stage.TRAIN:
            prior = getattr(self, 'scheduled_prior', self.hparams.boundary_predictor_prior)
        ...
        if flags.PRINT_FLOW:
            print(f"[pretrain_test.py] AFTER BoundaryPredictor:")
        if flags.PRINT_DATA:
            print(f"  enc.shape = {enc.shape}")
            print(f"  enc_lens.shape = {enc_lens.shape}")
            print(f"  enc_lens = {enc_lens}")
        ...
        if flags.PRINT_FLOW:
            print(f"[pretrain_test.py] BEFORE Transformer:")
        if flags.PRINT_DATA:
            print(f"  enc.shape = {enc.shape}")
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            scheduled_prior = getattr(self, 'scheduled_prior', self.hparams.boundary_predictor_prior)
            ...
            print(f"Binomial loss: {loss_boundary.item():.6f}, "
                  f"Scheduled compression: {scheduled_compression:.2f}x, "
                  f"Target compression: {target_compression:.2f}x, "
    ...
    def _compute_scheduled_prior(self, epoch):
        """Compute the scheduled prior for the current epoch.

        Schedule types:
            none              - Use target prior directly
            linear            - Linear 1x -> target over all epochs
            hold_then_linear  - Hold 1x for N epochs, then linear to target
            hold_at_target    - Ramp 1x -> target over N epochs, then hold
            step              - Discrete steps: "0:1.0,16:2.0,36:3.5,51:5.2"
        """
        schedule_type = getattr(self.hparams, 'compression_schedule_type', 'none')
        target_prior = self.hparams.boundary_predictor_prior
        target_compression = 1.0 / target_prior
        ...
        if schedule_type == 'none':
            ...
            if total_epochs > 1:
                progress = min(epoch, total_epochs - 1) / (total_epochs - 1)
            else:
                progress = 1.0
            compression = 1.0 + (target_compression - 1.0) * progress
            return 1.0 / compression

        elif schedule_type == 'hold_then_linear':
            hold_epochs = getattr(self.hparams, 'compression_hold_epochs', 5)
            if epoch < hold_epochs:
                return 1.0  # 1x compression
            remaining = total_epochs - hold_epochs
            if remaining > 1:
                progress = min(epoch - hold_epochs, remaining - 1) / (remaining - 1)
            else:
                progress = 1.0
            compression = 1.0 + (target_compression - 1.0) * progress
            return 1.0 / compression

        elif schedule_type == 'hold_at_target':
            ramp_epochs = getattr(self.hparams, 'compression_ramp_epochs', 10)
            ...
            steps_str = getattr(self.hparams, 'compression_steps', '')
            steps = []
            for pair in steps_str.split(','):
                pair = pair.strip()
                if ':' in pair:
                    ep_str, comp_str = pair.split(':')
                    steps.append((int(ep_str), float(comp_str)))
            steps.sort(key=lambda x: x[0])
            compression = 1.0
            for ep, comp in steps:
                if epoch >= ep:
                    compression = comp
            return 1.0 / max(compression, 1.0)

        else:
            raise ValueError(f"Unknown compression_schedule_type: {schedule_type}")
    ...
    def on_stage_start(self, stage, epoch):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            self.scheduled_prior = self._compute_scheduled_prior(epoch)
            scheduled_compression = 1.0 / self.scheduled_prior
            schedule_type = getattr(self.hparams, 'compression_schedule_type', 'none')

            logger.info(
                f"Epoch {epoch}: temperature = {temperature:.4f}, "
                f"compression = {scheduled_compression:.2f}x "
                f"(prior = {self.scheduled_prior:.4f}, schedule = {schedule_type})")
