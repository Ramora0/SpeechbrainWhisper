...
Boundaries are discrete, so during training we employ a differentiable Gumbel--Bernoulli estimator. Let $b_t \in \{0,1\}$ indicate a boundary \emph{after} frame $t$. We sample a relaxed boundary $\tilde{b}_t \in (0,1)$ as
\begin{equation}
\tilde{b}_t = \sigma\!\left(\frac{\log p_t - \log(1-p_t) + g_t}{\tau}\right),
\qquad g_t \sim \mathrm{Gumbel}(0,1),
\end{equation}
where $\tau>0$ is a temperature annealed from 1 to 0 over training. To obtain discrete boundaries in the forward pass while preserving gradients, we use a straight-through estimator:
\begin{equation}
b_t^{\mathrm{hard}} = \mathbb{I}[\tilde{b}_t > 0.5], \qquad
b_t = \mathrm{stopgrad}(b_t^{\mathrm{hard}} - \tilde{b}_t) + \tilde{b}_t.
\end{equation}
At inference time, we perform deterministic segmentation by thresholding:
\begin{equation}
b_t = \mathbb{I}[p_t > 0.5].
\end{equation}
...
\subsection{Rate control via binomial loss on sampled boundary counts}
\label{sec:lapse_rate_control}
A key goal of LAPSE is \emph{controllable} compression. We derive a target segment count $N^\star$ from the transcript using an external phonemizer, where $N^\star$ corresponds to either the phoneme count (phoneme-level) or syllable count (syllable-level). Since each boundary variable $b_t \in \{0,1\}$ indicates a split \emph{after} frame $t$, the number of segments is
\begin{equation}
...
Unlike approaches that regularize the \emph{probabilities} $p_t$, we apply a \textbf{binomial loss directly to the sampled boundary count} $B$ produced by the (straight-through) Gumbel--Bernoulli sampler (Section~\ref{sec:lapse_sampling}). Concretely, let the target number of boundaries be
\begin{equation}
B^\star \;=\; N^\star - 1,
\end{equation}
...
\;=\;
-\log \mathrm{BinomPMF}\!\left(B;\, T-1,\, \rho^\star\right)
\;=\;
-\log \left[
\binom{T-1}{B} (\rho^\star)^B (1-\rho^\star)^{(T-1-B)}
\right].
...
\paragraph{Backpropagation through sampled counts.}
Although $\mathcal{L}_{\mathrm{count}}$ is computed from the discrete sampled count $B=\sum_t b_t^{\mathrm{hard}}$, gradients flow to the boundary predictor through the straight-through construction used in sampling (Section~\ref{sec:lapse_sampling}), i.e.,
\[
b_t \;=\; \mathrm{stopgrad}(b_t^{\mathrm{hard}}-\tilde b_t)+\tilde b_t,
\]
so the forward pass uses discrete boundaries while the backward pass uses the relaxed samples $\tilde b_t$.

\paragraph{Targets.}
We consider two operating points:
(i) \textbf{phoneme-rate} targets, where $N^\star$ is the phoneme count (empirically targeting $\sim 10.4\times$ compression relative to the pre-LAPSE frame rate), and
(ii) \textbf{syllable-rate} targets, where $N^\star$ is the syllable count (targeting $\sim 28\times$ compression).



\subsection{Training Objective}
\label{sec:lapse_objective}
LAPSE is trained end-to-end with a standard CTC loss computed from the downstream encoder outputs and the transcript, augmented with the count loss:
\begin{equation}
\mathcal{L} = \mathcal{L}_{\mathrm{CTC}} + \lambda \, \mathcal{L}_{\mathrm{count}},
\end{equation}
where $\lambda$ is a scalar weight controlling the strength of rate control. We consider two operating points: a phoneme-level target regime (approximately $10.4\times$ compression relative to the pre-LAPSE frame rate) and a syllable-level regime (approximately $28\times$ compression).
